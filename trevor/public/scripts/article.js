...
//wrap everything in IIFE function that takes in module as param
...
(function(module){
  ...
  */
    //Rewrite this forEach using .map
  ...
  //use .map and .reduce to count the words of all articles combined
  Article.numWordsAll = () => {
    return Article.all.map((object) =>{
      return object.body.split(' ').length //you need to split the article.body with a space first. Then get length
    }).reduce( (acc, curr) => { //Then you add all together with reduce and return array with one number in it.
      return acc + curr;
    });
  };

  //produce array of unique authors. Many have published multiple times so I need to reduce them
  Article.allAuthors = () => {
    return Article.all.map((object) => {
      return object.author
    }).reduce((acc, curr) =>{
      if(!acc.includes(curr)){ //need to include if statement
        acc.push(curr);
      }
      return acc;
    },[]);
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map((author) => {//return object literal
      return{
        authorName: author,
        totalWords:
          Article.all.filter((object) => {
            return object.author === author;
          }).map((object) => {
            return object.body.split(' ').length;
          }).reduce((acc, curr) =>{
            return acc + curr;
          })
      }
